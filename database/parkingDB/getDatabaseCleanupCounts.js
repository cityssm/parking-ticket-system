...
export const getDatabaseCleanupCounts = () => {
    ...
    const parkingTicketStatusLog = database
        .prepare('select count(*) as cnt from ParkingTicketStatusLog' +
        ' where recordDelete_timeMillis is not null' +
        ' and recordDelete_timeMillis < ?')
        .get(recordDelete_timeMillisWindow).cnt;
    const parkingTicketRemarks = database
        .prepare('select count(*) as cnt from ParkingTicketRemarks' +
        ' where recordDelete_timeMillis is not null' +
        ' and recordDelete_timeMillis < ?')
        .get(recordDelete_timeMillisWindow).cnt;
    const licencePlateOwners = database
        .prepare('select count(*) as cnt from LicencePlateOwners' +
        ' where recordDelete_timeMillis is not null' +
        ' and recordDelete_timeMillis < ?')
        .get(recordDelete_timeMillisWindow).cnt;
    const parkingLocations = database
        .prepare('select count(*) as cnt from ParkingLocations l' +
        ' where isActive = 0' +
    ...
    const parkingBylaws = database
        .prepare('select count(*) as cnt from ParkingBylaws b' +
        ' where isActive = 0' +
    ...
    const parkingOffences = database
        .prepare('select count(*) as cnt from ParkingOffences o' +
        ' where isActive = 0' +
        (' and not exists (' +
