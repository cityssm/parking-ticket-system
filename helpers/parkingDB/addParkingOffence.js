...
export const addParkingOffence = (requestBody) => {
    const database = sqlite(databasePath);
    const existingOffenceRecord = database.prepare("select isActive" +
    ...
    if (Object.prototype.hasOwnProperty.call(requestBody, "offenceAmount")) {
        ...
        const offenceAmountRecord = database.prepare("select offenceAmount, discountOffenceAmount, discountDays" +
            " from ParkingOffences" +
            " where bylawNumber = ?" +
            " and isActive = 1" +
