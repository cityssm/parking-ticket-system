    ...
    describe('parking ticket queries', () => {
        describe('getParkingTickets()', () => {
            it('should execute with no filters', () => {
                assert.ok(parkingDB_getParkingTickets(fakeViewOnlySession.user, {
                    limit: 1,
                    offset: 0
                }));
            });
        ...
        it('should execute getParkingTicketRemarks(-1)', () => {
            assert.strictEqual(parkingDB_getParkingTicketRemarks(-1, fakeViewOnlySession.user).length, 0);
        });
        it('should execute getParkingTicketStatuses(-1)', () => {
            assert.strictEqual(parkingDB_getParkingTicketStatuses(-1, fakeViewOnlySession.user).length, 0);
        });
    });
    describe('licence plate queries', () => {
        it('should execute getLicencePlates()', () => {
            assert.ok(parkingDB_getLicencePlates({
                limit: 1,
                offset: 0
            }));
        });
