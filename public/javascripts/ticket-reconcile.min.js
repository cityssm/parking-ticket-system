...
function k(b){b.preventDefault();var f=b.currentTarget,d=f.closest("td"),a=d.closest("tr");cityssm.confirmModal("Clear Status","Are you sure you want to undo this status?","Yes, Remove the Status","warning",function(){cityssm.postJSON("/tickets/doDeleteStatus",{ticketID:a.getAttribute("data-ticket-id"),statusIndex:f.getAttribute("data-status-index")},function(a){a.success&&(cityssm.clearElement(d),d.classList.remove("has-width-200"),d.innerHTML='<button class="button is-success is-ownership-match-button" type="button"><span class="icon"><i class="fas fa-check" aria-hidden="true"></i></span><span>Match</span></button> <button class="button is-danger is-ownership-error-button" type="button"><i class="fas fa-times" aria-hidden="true"></i><span class="sr-only">Error</span></button>',
...
m);(c=document.getElementById("is-quick-reconcile-matches-button"))&&c.addEventListener("click",function(b){b.preventDefault();var e,d=function(){cityssm.postJSON("/tickets/doQuickReconcileMatches",{},function(a){e();if(a.success){cityssm.alertModal("Quick Reconcile Complete",1===a.statusRecords.length?"One record was successfully reconciled as a match.":a.statusRecords.length+" records were successfully reconciled as matches.","OK","success");for(var b=0;b<a.statusRecords.length;b+=1){var d=a.statusRecords[b],
