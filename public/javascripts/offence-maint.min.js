...
z.getElementsByTagName("table")[0].appendChild(a),c>d&&z.insertAdjacentHTML("afterbegin",'<div class="message is-warning"><div class="message-body has-text-centered">Limit Reached</div></div>'))};document.getElementById("is-add-offence-button").addEventListener("click",function(a){a.preventDefault();if(t&&y){a=r.get(v);var b=m.get(u);cityssm.confirmModal("Create Offence?",'<p class="has-text-centered">Are you sure you want to create the offence record below?</p><div class="columns has-margin-top-20 has-margin-bottom-20">'+
('<div class="column has-text-centered">'+cityssm.escapeHTML(b.locationName)+'<br /><span class="is-size-7">'+(k.has(b.locationClassKey)?k.get(b.locationClassKey).locationClass:b.locationClassKey)+"</span></div>")+('<div class="column has-text-centered">'+cityssm.escapeHTML(a.bylawNumber)+'<br /><span class="is-size-7">'+cityssm.escapeHTML(a.bylawDescription)+"</span></div>")+"</div>","Yes, Create Offence","info",function(){d(v,u,!0,function(a){a.success?a.message&&cityssm.alertModal("Offence Added Successfully",
a.message,"OK","warning"):cityssm.alertModal("Offence Not Added",a.message,"OK","danger")})})}else t||y?h():cityssm.alertModal("How to Add an Offence","To add an offence, use the main filters to select either a location, a by-law, or both.","OK","info")});C.addEventListener("dblclick",B);document.getElementById("is-select-location-filter-button").addEventListener("click",B);document.getElementById("is-clear-location-filter-button").addEventListener("click",function(){l();n()});t||l();D.addEventListener("dblclick",
