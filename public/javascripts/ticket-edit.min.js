...
c.value=a.offenceAmount;document.getElementById("ticket--parkingOffence").value=a.bylawDescription;z();q=[]},D=function(){var a=document.getElementById("ticket--locationKey").value;document.getElementById("ticket--locationName");pts.postJSON("/offences/doGetOffencesByLocation",{locationKey:a},function(a){q=a;a=document.createElement("div");a.className="list is-hoverable has-margin-bottom-20";for(var c=0;c<q.length;c+=1){var b=q[c],k=document.createElement("a");k.className="list-item";k.setAttribute("data-index",
c);k.setAttribute("href","#");k.addEventListener("click",C);k.innerHTML='<div class="columns">'+('<div class="column">'+pts.escapeHTML(b.bylawNumber)+"<br /><small>"+pts.escapeHTML(b.bylawDescription)+"</small></div>")+('<div class="column is-narrow">$ '+b.offenceAmount.toFixed(2)+"</div>")+"</div>";a.insertAdjacentElement("beforeend",k)}c=document.getElementById("container--bylawNumbers");pts.clearElement(c);c.appendChild(a)})},g=function(a){a.preventDefault();pts.openHtmlModal("ticket-setBylawOffence",
{onshown:function(a,b){z=b;D();document.getElementById("bylawLookup--searchStr").focus()},onremoved:function(){document.getElementById("is-bylaw-lookup-button").focus()}})};document.getElementById("is-bylaw-lookup-button").addEventListener("click",g);document.getElementById("ticket--bylawNumber").addEventListener("dblclick",g);g=function(){var a=document.getElementById("datalist--licencePlateProvince");pts.clearElement(a);var c=pts.getLicencePlateCountryProperties(document.getElementById("ticket--licencePlateCountry").value);
if(c&&c.provinces)for(var c=Object.values(c.provinces),b=0;b<c.length;b+=1){var e=document.createElement("option");e.setAttribute("value",c[b].provinceShortName);a.appendChild(e)}};document.getElementById("ticket--licencePlateCountry").addEventListener("change",g);pts.loadDefaultConfigProperties(g);var u=function(a){for(a=a.getElementsByClassName("panel-block");0<a.length;)a[0].remove()};if(!f){var r=document.getElementById("is-remark-panel"),t=pts.ticketRemarksInit;delete pts.ticketRemarksInit;var v,
E=function(a){var c=a.currentTarget.getAttribute("data-remark-index");pts.confirmModal("Delete Remark?","Are you sure you want to delete this remark?","Yes, Delete","warning",function(){pts.postJSON("/tickets/doDeleteRemark",{ticketID:b,remarkIndex:c},function(a){a.success&&v()})})},F=function(a){a.preventDefault();var c;a=parseInt(a.currentTarget.getAttribute("data-index"));var m=t[a],e=function(a){a.preventDefault();pts.postJSON("/tickets/doUpdateRemark",a.currentTarget,function(a){a.success&&(c(),
v())})};pts.openHtmlModal("ticket-editRemark",{onshow:function(a){document.getElementById("editRemark--ticketID").value=b;document.getElementById("editRemark--remarkIndex").value=m.remarkIndex;document.getElementById("editRemark--remark").value=m.remark;document.getElementById("editRemark--remarkDateString").value=m.remarkDateString;document.getElementById("editRemark--remarkTimeString").value=m.remarkTimeString;a.getElementsByTagName("form")[0].addEventListener("submit",e)},onshown:function(a,b){c=
b}})},A=function(){u(r);if(0===t.length)r.insertAdjacentHTML("beforeend",'<div class="panel-block is-block"><div class="message is-info"><p class="message-body">There are no remarks associated with this ticket.</p></div></div>');else for(var a=0;a<t.length;a+=1){var c=t[a],b=document.createElement("div");b.className="panel-block is-block";b.innerHTML='<div class="columns">'+('<div class="column"><p class="has-newline-chars">'+pts.escapeHTML(c.remark)+'</p><p class="is-size-7">'+(c.recordCreate_timeMillis===
...
c.remarkIndex+'" type="button"><i class="fas fa-trash" aria-hidden="true"></i><span class="sr-only">Delete</span></button></div></div>':"")+"</div>";c.canUpdate&&(b.getElementsByClassName("is-edit-remark-button")[0].addEventListener("click",F),b.getElementsByClassName("is-delete-remark-button")[0].addEventListener("click",E));r.appendChild(b)}};v=function(){u(r);r.insertAdjacentHTML("beforeend",'<div class="panel-block is-block"><p class="has-text-centered has-text-grey-lighter"><i class="fas fa-2x fa-circle-notch fa-spin" aria-hidden="true"></i><br /><em>Loading remarks...</p></div>');
pts.postJSON("/tickets/doGetRemarks",{ticketID:b},function(a){t=a;A()})};document.getElementById("is-add-remark-button").addEventListener("click",function(a){a.preventDefault();var c,d=function(a){a.preventDefault();pts.postJSON("/tickets/doAddRemark",a.currentTarget,function(a){a.success&&(c(),v())})};pts.openHtmlModal("ticket-addRemark",{onshow:function(a){document.getElementById("addRemark--ticketID").value=b;a.getElementsByTagName("form")[0].addEventListener("submit",d)},onshown:function(a,b){c=
b}})});A()}if(!f){var p=document.getElementById("is-status-panel"),w=pts.ticketStatusLogInit;delete pts.ticketStatusLogInit;var x,G=function(a){a.preventDefault();pts.confirmModal("Mark Ticket as Resolved?","Once resolved, you will no longer be able to make changes to the ticket.","Yes, Resolve Ticket","info",function(){pts.postJSON("/tickets/doResolveTicket",{ticketID:b},function(a){a.success&&(window.location.href="/tickets/"+b)})})},H=function(a){var c=a.currentTarget.getAttribute("data-status-index");
pts.confirmModal("Delete Remark?","Are you sure you want to delete this status?","Yes, Delete","warning",function(){pts.postJSON("/tickets/doDeleteStatus",{ticketID:b,statusIndex:c},function(a){a.success&&x()})})},I=function(a){},B=function(){u(p);if(0===w.length)p.insertAdjacentHTML("beforeend",'<div class="panel-block is-block"><div class="message is-info"><p class="message-body">There are no statuses associated with this ticket.</p></div></div>');else for(var a=0;a<w.length;a+=1){var b=w[a],d=
pts.getTicketStatus(b.statusKey);if(0===a&&d&&d.isFinalStatus){var e=document.createElement("div");e.className="panel-block is-block";e.innerHTML='<div class="message is-info is-clearfix"><div class="message-body"><div class="columns"><div class="column"><strong>This ticket is able to be marked as resolved.</strong></div><div class="column is-narrow"><button class="button is-info" type="button"><span class="icon is-small"><i class="fas fa-check" aria-hidden="true"></i></span><span>Resolve Ticket</span></button></div></div></div></div>';
e.getElementsByTagName("button")[0].addEventListener("click",G);p.appendChild(e)}e=document.createElement("div");e.className="panel-block is-block";e.innerHTML='<div class="columns">'+('<div class="column">'+('<div class="level has-margin-bottom-5"><div class="level-left"><strong>'+(d?d.status:b.statusKey)+'</strong></div><div class="level-right">'+b.statusDateString+"</div></div>")+(""===b.statusField?"":'<p class="is-size-7"><strong>'+(d&&d.statusField?d.statusField.fieldLabel:"")+":</strong> "+
b.statusField+"</p>")+'<p class="is-size-7">'+b.statusNote+"</p></div>")+(b.canUpdate&&0===a?'<div class="column is-narrow"><div class="buttons is-right has-addons"><button class="button is-small is-edit-status-button" data-tooltip="Edit Status" data-index="'+a+'" type="button"><span class="icon is-small"><i class="fas fa-pencil-alt" aria-hidden="true"></i></span> <span>Edit</span></button><button class="button is-small has-text-danger is-delete-status-button" data-tooltip="Delete Status" data-status-index="'+
b.statusIndex+'" type="button"><i class="fas fa-trash" aria-hidden="true"></i><span class="sr-only">Delete</span></button></div></div>':"")+"</div>";b.canUpdate&&0===a&&(e.getElementsByClassName("is-edit-status-button")[0].addEventListener("click",I),e.getElementsByClassName("is-delete-status-button")[0].addEventListener("click",H));p.appendChild(e)}};x=function(){u(p);p.insertAdjacentHTML("beforeend",'<div class="panel-block is-block"><p class="has-text-centered has-text-grey-lighter"><i class="fas fa-2x fa-circle-notch fa-spin" aria-hidden="true"></i><br /><em>Loading statuses...</p></div>');
pts.postJSON("/tickets/doGetStatuses",{ticketID:b},function(a){w=a;B()})};document.getElementById("is-add-status-button").addEventListener("click",function(a){a.preventDefault();var c,d=function(a){a.preventDefault();var d=document.getElementById("addStatus--resolveTicket").checked;pts.postJSON("/tickets/doAddStatus",a.currentTarget,function(a){a.success&&(c(),d?window.location.href="/tickets/"+b:x())})},e=function(a){a=pts.getTicketStatus(a.currentTarget.value);var b=document.getElementById("addStatus--statusField");
...
function(a){for(var b=document.getElementById("addStatus--statusKey"),c=0;c<a.length;c+=1){var d=a[c];d.isUserSettable&&b.insertAdjacentHTML("beforeend",'<option value="'+d.statusKey+'">'+d.status+"</option>")}b.addEventListener("change",e)});a.getElementsByTagName("form")[0].addEventListener("submit",d)},onshown:function(a,b){c=b}})});pts.loadDefaultConfigProperties(B)}for(var g=function(a){a.preventDefault();a=a.currentTarget;var b=a.getAttribute("data-unlock"),b=a.closest(".field").getElementsByTagName(b)[0];
b.removeAttribute("readonly");b.classList.remove("is-readonly");b.focus();a.setAttribute("disabled","disabled")},n=document.getElementsByClassName("is-unlock-field-button"),y=0;y<n.length;y+=1)n[y].addEventListener("click",g)})();
