...
export const markConvictionBatchAsSent = (batchID, requestSession) => {
    ...
    database.prepare("update ParkingTickets" +
        " set resolvedDate = ?," +
        " recordUpdate_userName = ?," +
        " recordUpdate_timeMillis = ?" +
        ...
        (" and exists (" +
            ...
            " and s.statusField = ?" +
            ")")).run(dateTimeFns.dateToInteger(rightNow), requestSession.user.userName, rightNow.getTime(), batchID.toString());
    database.close();
