(function(){function y(a){a.preventDefault();var b=a.currentTarget;b.setAttribute("disabled","disabled");var c=parseInt(b.getAttribute("data-index"));a=p[c];var g=b.closest(".is-plate-container");pts.postJSON("/plates/doAddLicencePlateToLookupBatch",{batchID:d,licencePlateCountry:"CA",licencePlateProvince:"ON",licencePlateNumber:a.licencePlateNumber,ticketID:a.ticketIDMin},function(a){a.success?(g.remove(),p[c]=null,k(a.batch)):b.removeAttribute("disabled")})}function z(a){a.preventDefault();var b=
...
"Yes, Clear the Batch","warning",function(){pts.postJSON("/plates/doClearLookupBatch",{batchID:d},function(a){a.success&&(k(a.batch),l())})})}function B(a){a.preventDefault();a=function(){window.open("/plates/mtoExport/"+d);r=!0};r?a():pts.confirmModal("Download Batch?","<strong>You are about to download this batch for the first time.</strong><br />The date of this download will be tracked as part of the batch record.","OK, Download the File","warning",a)}function C(a,b){return a+'<a class="tag has-tooltip-bottom" data-tooltip="View Ticket (Opens in New Window)" href="/tickets/byTicketNumber/'+
...
function D(){pts.postJSON("/plates/doGetLookupBatch",{batchID:d},function(a){k(a);l()})}var w="true"===document.getElementsByTagName("main")[0].getAttribute("data-can-update"),d=-1,h=!0,r=!1,k,l,x=document.getElementById("available--issueDaysAgo"),n=document.getElementById("is-available-plates-container"),v=document.getElementById("available--licencePlateNumber"),p=[],q=[];l=function(){h?n.innerHTML='<div class="message is-info"><div class="message-body">Licence Plates cannot be added to a locked batch.</div></div>':
...
a.batchDateString+"</span>"+(h?' <span class="tag is-light"><span class="icon is-small"><i class="fas fa-lock" aria-hidden="true"></i></span> <span>'+a.lockDateString+"</span></span>":"");pts.clearElement(e);if(0===q.length)e.innerHTML='<div class="message is-info"><p class="message-body">There are no licence plates included in the batch.</p></div>';else{pts.clearElement(e);a=document.createElement("div");a.className="panel";for(var b=0;b<q.length;b+=1){var c=q[b],g=document.createElement("div");
g.className="panel-block is-block is-entry-container";g.innerHTML='<div class="level">'+('<div class="level-left"><div class="licence-plate"><div class="licence-plate-number">'+pts.escapeHTML(c.licencePlateNumber)+"</div></div></div>")+(h?"":'<div class="level-right"><button class="button is-small" data-index="'+b+'" type="button"><span class="icon is-small"><i class="fas fa-minus" aria-hidden="true"></i></span><span>Remove</span></button></div>')+"</div>";h||g.getElementsByTagName("button")[0].addEventListener("click",
...
(b=document.createElement("button"),b.className="button is-fullwidth has-margin-bottom-10",b.innerHTML='<span class="icon is-small"><i class="fas fa-broom" aria-hidden="true"></i></span><span>Clear Batch</span>',b.addEventListener("click",A),e.appendChild(b));e.appendChild(a)}};document.getElementById("is-select-batch-button").addEventListener("click",function(a){a.preventDefault();var b,c,g=function(a){a.preventDefault();d=parseInt(a.currentTarget.getAttribute("data-batch-id"));b();D()},f=function(){pts.postJSON("/plates/doGetUnreceivedLicencePlateLookupBatches",
...
{onshow:function(a){c=a.getElementsByClassName("is-results-container")[0];f()},onshown:function(a,c){b=c}})});w&&(document.getElementById("is-create-batch-button").addEventListener("click",function(){pts.confirmModal("Create a New Batch?","Are you sure you want to create a new licence plate lookup batch?","Yes, Create a Batch","info",function(){pts.postJSON("/plates/doCreateLookupBatch",{},function(a){a.success&&(k(a.batch),l())})})}),t.addEventListener("click",function(){h||pts.confirmModal("Lock Batch?",
'<strong>Are you sure you want to lock the batch?</strong><br />Once the batch is locked, no licence plates can be added or deleted from the batch. All tickets related to the licence plates in the batch will be updated with a "Pending Lookup" status.',"Yes, Lock the Batch","info",function(){pts.postJSON("/plates/doLockLookupBatch",{batchID:d},function(a){a.success&&k(a.batch)})})}));pts.plateExportBatch&&(k(pts.plateExportBatch),delete pts.plateExportBatch,l())})();
