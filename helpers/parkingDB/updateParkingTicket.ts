...
export const getLicencePlateExpiryDateFromPieces =
  (requestBody: pts.ParkingTicket): { success: boolean; message?: string; licencePlateExpiryDate?: number; } => {

    let licencePlateExpiryDate = 0;

    const licencePlateExpiryYear = Number.parseInt(requestBody.licencePlateExpiryYear as string, 10) || 0;
    const licencePlateExpiryMonth = Number.parseInt(requestBody.licencePlateExpiryMonth as string, 10) || 0;
    ...
    if (licencePlateExpiryYear === 0 && licencePlateExpiryMonth === 0) {
      ...
      const dateObject = new Date(licencePlateExpiryYear,
...
export const updateParkingTicket =
  (requestBody: pts.ParkingTicket, requestSession: expressSession.Session): { success: boolean; message?: string; } => {
