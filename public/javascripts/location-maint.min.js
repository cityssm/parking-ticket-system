...
a),r.appendChild(n))}}pts.clearElement(h);0===e?h.innerHTML='<div class="message is-info"><div class="message-body">There are no locations that meet your search criteria.</div></div>':(h.innerHTML='<table class="table is-fixed is-striped is-hoverable is-fullwidth"><thead><tr><th>Location</th><th>Class</th></tr></thead></table>',h.getElementsByTagName("table")[0].appendChild(r))};c.addEventListener("change",g);d.addEventListener("keyup",g);pts.getDefaultConfigProperty("locationClasses",function(a){c.innerHTML=
'<option value="">(All Location Classes)</option>';for(var b=0;b<a.length;b+=1){var d=a[b];f+='<option value="'+d.locationClassKey+'">'+pts.escapeHTML(d.locationClass)+"</option>";k.set(d.locationClassKey,d)}c.insertAdjacentHTML("beforeend",f);g()});document.getElementById("is-add-location-button").addEventListener("click",function(a){a.preventDefault();var c,d=function(a){a.preventDefault();pts.postJSON("/admin/doAddLocation",a.currentTarget,function(a){a.success?(c(),b=a.locations,g()):pts.alertModal("Location Not Added",
a.message,"OK","danger")})};pts.openHtmlModal("location-add",{onshown:function(a,b){c=b;document.getElementById("addLocation--locationClassKey").insertAdjacentHTML("beforeend",f);document.getElementById("form--addLocation").addEventListener("submit",d)}})})})();
