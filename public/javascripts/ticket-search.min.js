...
"</div>")+"</div></td>")+("<td>"+(a.locationDescription?pts.escapeHTML(a.locationDescription)+"<br />":"")+(a.locationName?'<small class="has-tooltip-right" data-tooltip="'+pts.escapeHTML(r)+'"><i class="fas fa-map-marker-alt" aria-hidden="true"></i> '+a.locationName+"</small>":"")+"</td>")+"<td>"+pts.escapeHTML(a.parkingOffence)+"</td><td>"+(""===a.resolvedDateString?"Unresolved":'<span class="sr-only">Resolved</span><i class="fas fa-check" aria-hidden="true"></i> '+a.resolvedDateString)+(a.latestStatus_statusKey?
'<br /><span class="tag is-light is-primary">'+m.status+"</span>":"")+"</td>";h.insertAdjacentElement("beforeend",k)}g.insertAdjacentHTML("beforeend",'<div class="level is-block-print"><div class="level-left has-text-weight-bold">Displaying parking tickets '+(e+1)+" to "+Math.min(f+e,b.count)+" of "+b.count+"</div></div>");f<b.count&&(c=document.createElement("nav"),c.className="level-right is-hidden-print",c.setAttribute("role","pagination"),c.setAttribute("aria-label","pagination"),0<e&&(h=document.createElement("a"),
h.className="button",h.innerText="Previous",h.addEventListener("click",function(a){a.preventDefault();l.value=Math.max(0,e-f);n()}),c.insertAdjacentElement("beforeend",h)),f+e<b.count&&(b=document.createElement("a"),b.className="button has-margin-left-10",b.innerHTML='<span>Next Licences</span><span class="icon"><i class="fas fa-chevron-right" aria-hidden="true"></i></span>',b.addEventListener("click",function(a){a.preventDefault();l.value=e+f;n()}),c.insertAdjacentElement("beforeend",b)),g.getElementsByClassName("level")[0].insertAdjacentElement("beforeend",
c))}})}function d(){l.value=0;n()}var t={},q="",k=document.getElementById("form--filters"),u=document.getElementById("filter--limit"),l=document.getElementById("filter--offset"),g=document.getElementById("container--searchResults");k.addEventListener("submit",function(d){d.preventDefault()});document.getElementById("filter--ticketNumber").addEventListener("change",d);document.getElementById("filter--licencePlateNumber").addEventListener("change",d);document.getElementById("filter--location").addEventListener("change",
